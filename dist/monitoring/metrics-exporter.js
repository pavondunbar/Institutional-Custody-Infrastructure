...
class MetricsExporter {
    ...
    async collect() {
        const metrics = [];
        // --- Database metrics ---
        const dbStats = await connection_1.db.query(`SELECT count(*) as active FROM pg_stat_activity WHERE state='active'`);
        metrics.push({ name: 'db_connections_active', help: 'Active database connections', type: 'gauge', value: parseInt(dbStats.rows[0].active) });
        // --- Ledger metrics ---
        const journalCount = await connection_1.db.query(`SELECT count(*)::int as c FROM journal_entries`);
        metrics.push({ name: 'ledger_journal_entries_total', help: 'Total journal entries', type: 'counter', value: journalCount.rows[0].c });
        const ledgerCount = await connection_1.db.query(`SELECT count(*)::int as c FROM ledger_entries`);
        metrics.push({ name: 'ledger_entries_total', help: 'Total ledger entries', type: 'counter', value: ledgerCount.rows[0].c });
        // --- Outbox metrics ---
        const outboxPending = await connection_1.db.query(`SELECT count(*)::int as c FROM outbox WHERE published=FALSE`);
        metrics.push({ name: 'outbox_pending_messages', help: 'Unpublished outbox messages', type: 'gauge', value: outboxPending.rows[0].c });
        // --- DLQ metrics ---
        const dlqStats = await connection_1.db.query(`SELECT status, count(*)::int as c FROM dead_letter_queue GROUP BY status`);
        ...
        // --- Transaction metrics ---
        const txStats = await connection_1.db.query(`SELECT status, count(*)::int as c FROM transactions_blockchain GROUP BY status`);
        ...
        // --- Wallet metrics ---
        const walletStats = await connection_1.db.query(`SELECT wallet_type, count(*)::int as c FROM wallets GROUP BY wallet_type`);
        ...
        // --- Risk metrics ---
        const riskEvents = await connection_1.db.query(`SELECT severity, count(*)::int as c FROM risk_events WHERE resolved=FALSE GROUP BY severity`);
        ...
        // --- Alert metrics ---
        const alertsUnresolved = await connection_1.db.query(`SELECT severity, count(*)::int as c FROM alert_events WHERE resolved=FALSE GROUP BY severity`);
        ...
        // --- Settlement metrics ---
        const settlementStats = await connection_1.db.query(`SELECT status, count(*)::int as c FROM settlement_instructions GROUP BY status`);
        ...
        const lastRecon = await connection_1.db.query(`SELECT run_type, status, started_at FROM reconciliation_runs ORDER BY started_at DESC LIMIT 5`);
        ...
        // --- Lending metrics ---
        try {
            const loanStats = await connection_1.db.query(`SELECT status, count(*)::int as c FROM loans GROUP BY status`);
