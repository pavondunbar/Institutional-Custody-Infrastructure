...
jest.mock('./connection', () => ({
    db: { query: jest.fn() },
    withSerializableTransaction: async (fn) => fn(mockClient),
}));
jest.mock('../config', () => ({
...
const mockDbQuery = connection_1.db.query;
describe('Ledger Service', () => {
    ...
    describe('postJournal', () => {
        it('rejects unbalanced journals before hitting the database', async () => {
            await expect((0, ledger_service_1.postJournal)({ idempotencyKey: 'bad', lines: [{ accountId: accountA, amount: 500n, direction: 'debit' }, { accountId: accountB, amount: 300n, direction: 'credit' }] })).rejects.toThrow('Unbalanced journal');
            expect(mockClientQuery).not.toHaveBeenCalled();
        });
        it('rejects empty journal lines', async () => {
            await expect((0, ledger_service_1.postJournal)({ idempotencyKey: 'empty', lines: [] })).rejects.toThrow('at least one line');
        });
        it('posts a balanced double-entry journal with hash chain', async () => {
            mockClientQuery
                ...
                // Outbox
                .mockResolvedValueOnce({});
            const result = await (0, ledger_service_1.postJournal)(balancedRequest);
            ...
            const ledgerInsert = mockClientQuery.mock.calls.find((c) => typeof c[0] === 'string' && c[0].includes('INSERT INTO ledger_entries'));
        ...
        it('returns existing journal on duplicate idempotency key', async () => {
            ...
            const journalInserts = mockClientQuery.mock.calls.filter((c) => typeof c[0] === 'string' && c[0].includes('INSERT INTO journal_entries'));
        ...
        it('validates hash chain continuity (prev_hash links to previous entry)', async () => {
            ...
            await (0, ledger_service_1.postJournal)(balancedRequest);
            // Find first ledger INSERT call
            const ledgerInserts = mockClientQuery.mock.calls.filter((c) => typeof c[0] === 'string' && c[0].includes('INSERT INTO ledger_entries'));
            const params = ledgerInserts[0][1];
            expect(params[5]).toBe(prevHash); // prev_hash param
            // Verify computed hash matches expected
            const expectedHash = (0, crypto_1.createHash)('sha256').update(`${prevHash}|${accountA}|1000|debit|4`).digest('hex');
    ...
    describe('reverseJournal', () => {
        it('rejects reversal of non-existent journal', async () => {
            ...
            await expect((0, ledger_service_1.reverseJournal)('nonexistent', 'rev-key')).rejects.toThrow('Journal entry not found');
        });
        it('rejects reversal of already-reversed journal', async () => {
            mockClientQuery.mockResolvedValueOnce({ rows: [{ id: 'j1', status: 'reversed' }] });
            await expect((0, ledger_service_1.reverseJournal)('j1', 'rev-key')).rejects.toThrow('already reversed');
        });
        it('posts mirror entries (debit↔credit swap)', async () => {
            mockClientQuery
                ...
                .mockResolvedValueOnce({})
                .mockResolvedValueOnce({}); // outbox
            const result = await (0, ledger_service_1.reverseJournal)('j1', 'rev-key-001');
    ...
    describe('getBalance', () => {
        it('returns null for unknown account', async () => {
            ...
            expect(await (0, ledger_service_1.getBalance)('unknown')).toBeNull();
        });
        it('returns balance with BigInt values', async () => {
            ...
            const result = await (0, ledger_service_1.getBalance)(accountA);
    ...
    describe('reconstructBalance', () => {
        it('independently derives balance from raw entries', async () => {
            ...
            const result = await (0, ledger_service_1.reconstructBalance)(accountA);
    ...
    describe('double-spend prevention', () => {
        it('serialization failure rejects concurrent conflicting transactions', async () => {
            ...
            await expect((0, ledger_service_1.postJournal)(balancedRequest)).rejects.toThrow('could not serialize access');
        });
    });
});
