...
export class AmlService {
  ...
  async screenAddress(address: string): Promise<ScreeningResult> {
    ...
    const result = await db.query(
      `SELECT id, list_type, entity_name, entity_type
    ...
    if (result.rows.length > 0) {
      ...
      await this.recordScreening(
        'address', address, match.id, 100, 'exact', 'flagged'
      );

      logger.warn(
        { address: normalizedAddr, list: match.list_type },
   ...
   */
  async screenEntity(name: string): Promise<ScreeningResult> {
    const result = await db.query(
      `SELECT id, list_type, entity_name, entity_type,
    ...
    if (result.rows.length > 0 && parseFloat(result.rows[0].sim_score) > 0.6) {
      ...
      await this.recordScreening(
        'counterparty', name, match.id, score, 'fuzzy', 'flagged'
      );
  ...
  async screenTransaction(params: {
    ...
    if (indicators.length > 0) {
      await this.recordScreening(
        'transaction', params.accountId, null, 0, null, 'flagged',
        { indicators, amount: params.amount.toString() }
      );
    }
  ...
  async fileSar(params: {
    ...
    await db.query(
      `INSERT INTO outbox (aggregate_type, aggregate_id, event_type, payload)
       VALUES ('compliance', $1, 'sar.filed', $2)`,
      [sarId, JSON.stringify({ sarId, subjectType: params.subjectType })]
    );

    logger.warn({ sarId }, 'Suspicious activity report filed');
    return sarId;
  }
  ...
  async getSars(status?: string, limit = 50) {
  ...
  async getScreeningResults(subjectId: string) {
    const result = await db.query(
