...
export class AuditService {
  ...
  async query(filters: {
    ...
    if (filters.actorId) addFilter('actor_id', filters.actorId);
    if (filters.resourceType) addFilter('resource_type', filters.resourceType);
    if (filters.resourceId) addFilter('resource_id', filters.resourceId);
    if (filters.eventType) addFilter('event_type', filters.eventType);
    if (filters.riskLevel) addFilter('risk_level', filters.riskLevel);
    if (filters.outcome) addFilter('outcome', filters.outcome);
    if (filters.correlationId) addFilter('correlation_id', filters.correlationId);
  ...
  async generateComplianceReport(
    ...
    const authFailures = await db.query(
    ...
    const deniedCount = await db.query(
      `SELECT COUNT(*) as count FROM audit_events
       WHERE created_at >= $1 AND created_at <= $2
         AND outcome = 'denied'`,
      [fromDate, toDate]
