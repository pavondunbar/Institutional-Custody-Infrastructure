...
export class RiskService {
  ...
  async evaluateTransaction(params: {
    ...
    const policies = await db.query(
      `SELECT * FROM risk_policies WHERE active = TRUE
  ...
  private async checkVelocity(
    ...
    params: { accountId: string; amount: bigint },
  ): Promise<RiskViolation | null> {
    const windowMinutes = (policy.config.window_minutes as number) || 60;
    const maxCount = policy.config.max_count as number | undefined;
    const maxAmount = policy.config.max_amount
      ? BigInt(policy.config.max_amount as string)
      : undefined;
  ...
  private async checkConcentration(
    ...
    params: { accountId: string; amount: bigint; tokenId?: string },
  ): Promise<RiskViolation | null> {
    if (!params.tokenId) return null;

    const maxBps = (policy.config.max_concentration_bps as number) || 5000;

    const token = await db.query(
      'SELECT total_minted, total_burned FROM token_definitions WHERE id = $1',
      [params.tokenId]
    );
    ...
    const holder = await db.query(
      'SELECT balance FROM token_holders WHERE token_id = $1 AND account_id = $2',
      [params.tokenId, params.accountId]
    );
  ...
  private async checkExposure(
    ...
    const maxExposure = policy.config.max_exposure
      ? BigInt(policy.config.max_exposure as string)
      : undefined;

    if (!maxExposure) return null;

    const balance = await db.query(
      'SELECT COALESCE(SUM(balance), 0) as total FROM balance_cache WHERE account_id = $1',
      [params.accountId]
    );
  ...
  async createPolicy(params: {
    ...
    const result = await db.query(
      `INSERT INTO risk_policies (name, policy_type, config, severity, action_on_breach)
       VALUES ($1, $2, $3, $4, $5) RETURNING id`,
      [params.name, params.policyType, JSON.stringify(params.config),
  ...
  async getPolicies() {
    const result = await db.query(
      'SELECT * FROM risk_policies ORDER BY severity DESC, created_at DESC'
    );
    return result.rows;
  }
  ...
  async resolveEvent(
    ...
    await db.query(
      `UPDATE risk_events SET resolved = TRUE, resolved_by = $1,
